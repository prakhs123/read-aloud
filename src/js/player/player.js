...
(function() {
  ...
  async function play() {
    try {
      currentIndex = await messagingClient.sendTo("content-script", {method: "getCurrentIndex"})
      return readCurrent()
    }
    catch (err) {
      console.error(err)
      reportStatus({status: "IDLE", error: wrapError(err)})
  ...
  async function readCurrent(rewinded) {
    const texts = await messagingClient.sendTo("content-script", {method: "getTexts", index: currentIndex}).catch(err => null)
    if (texts) {
      if (texts.length) {
        foundText = true;
        return read(texts);
      ...
      if (!foundText) reportStatus({status: "IDLE", error: {code: "error_no_text"}})
      else reportStatus({status: "IDLE"})
    }
