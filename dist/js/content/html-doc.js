...
var readAloudDoc = new function () {
  ...
  this.getTexts = function (index) {
    if (index == 0) return parse();else return null;
  };
  function parse() {
  ...
  function findTextBlocks(threshold) {
    ...
    var walk = function () {
      if ($(this).is("frame, iframe")) try {
      ...
      } catch (err) {} else if ($(this).is("dl")) addBlock(this);else if ($(this).is("ol, ul")) {
        var items = $(this).children().get();
        if (items.some(hasTextNodes)) addBlock(this);else if (items.some(hasParagraphs)) addBlock(this, true);else if (items.some(containsTextBlocks)) addBlock(this, true);
        ...
        if (hasTextNodes(this)) addBlock(this);else if (hasParagraphs(this)) addBlock(this, true);else $(this).children(":not(" + skipTags + ")").each(walk);
      }
    };
    var textBlocks = [];
  ...
  function getTexts(elem) {
    ...
    var texts = $(elem).data("read-aloud-multi-block") ? $(elem).children(":visible").get().map(getText) : getText(elem).split(paragraphSplitter);
  ...
  function addNumbering() {
    ...
    if (text && !text.match(/^[(]?(\d|[a-zA-Z][).])/)) children.each(function (index) {
