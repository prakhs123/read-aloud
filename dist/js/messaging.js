...
function registerMessagingPeer(name, handlers) {
  ...
  return {
    async sendTo(dest, message) {
      ...
      let response;
      if (tabDests.has(dest)) response = await brapi.tabs.sendMessage(await getTargetTabId(), message);else response = await brapi.runtime.sendMessage(message);
