...
function Speech(texts, options) {
  ...
  var ready = Promise.resolve(pickEngine()).then(function (x) {
    engine = x;
  ...
  function getState() {
    ...
    return new Promise(function (fulfill) {
      engine.isSpeaking(function (isSpeaking) {
        if (state == "PLAYING") fulfill(isSpeaking ? "PLAYING" : "LOADING");else fulfill("PAUSED");
      });
    });
  }
  ...
  function play() {
    if (index >= texts.length) {
      ...
      return ready.then(function () {
        return speak(texts[index], function () {
          state = "IDLE";
  ...
  function canPause() {
    return engine.pause && !(isChromeOSNative(options.voice) || options.voice.voiceName == "US English Female TTS (by Google)");
  }
  function pause() {
    return ready.then(function () {
      if (canPause()) {
  ...
  function forward() {
    if (index + 1 < texts.length) {
      index++;
      if (state == "PLAYING") return delayedPlay();else return stop();
  ...
  function rewind() {
    if (state == "PLAYING" && new Date().getTime() - state.startTime > 3 * 1000) {
      ...
      index--;
      if (state == "PLAYING") return stop().then(play);else return stop();
